...
async function callGoogleTTS(text) {
     ...
     const payload = { input: { text: text }, voice: { languageCode: 'de-DE', name: 'de-DE-Wavenet-F' }, audioConfig: { audioEncoding: 'MP3' } };
...
window.explainSimply = async function(event) {
    ...
    const content = flashcardState.cardContent[cardId];
    const prompt = `Erkläre das folgende Konzept aus der digitalen Bildverarbeitung in einfachen Worten, auf Deutsch, und nutze Markdown zur Formatierung. Thema: "${content.question}"\nInhalt:\n${content.answer.replace(/<[^>]*>/g, ' ')}`;
    try {
...
async function suggestTopic() {
    ...
    const prompt = `Basierend auf den existierenden Lernkarten-Themen (${existingTopics}), schlage ein verwandtes, aber neues Thema im Bereich der digitalen Bildverarbeitung vor. Antworte nur mit dem Namen des Themas.`;
